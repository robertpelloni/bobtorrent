...
// Load Config
let config = {
  nodeName: 'Reference-Node-JS',
  storagePath: STORAGE_DIR,
  ports: { webApi: 3000, dht: 6881, tcp: 0 },
  transports: { clearnet: true, i2p: false, tor: false },
  storage: { enableErasure: false, defaultDataShards: 4, defaultParityShards: 2 }
}
...
async function handleApi (req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (route.startsWith('files/') && route.endsWith('/repair')) {
      const fileId = route.substring(6, route.length - 7)
      res.writeHead(200, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        fileId,
        success: false,
        message: 'Erasure coding repair is not supported on the Node.js Reference Client (Requires Java Supernode)',
        shardsRepaired: 0
      }))
      return
    }
