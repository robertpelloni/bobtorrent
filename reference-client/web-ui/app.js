...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function updateFiles () {
    ...
    window.inspectFile = async (id, name) => {
        ...
        btnRepair.onclick = async () => {
            ...
            try {
                const res = await apiFetch(`/api/files/${id}/repair`, { method: 'POST' });
                const result = await res.json();
                ...
                btnRepair.textContent = 'Repair Shards';
  ...
  const confPath = document.getElementById('conf-path');
  const confClearnet = document.getElementById('conf-clearnet');
  const confI2P = document.getElementById('conf-i2p');
  const confTor = document.getElementById('conf-tor');
  const confErasure = document.getElementById('conf-erasure');
  const confDataShards = document.getElementById('conf-data-shards');
  const confParityShards = document.getElementById('conf-parity-shards');
  ...
  async function loadConfig() {
      try {
          ...
          confName.value = config.nodeName || '';
          confPath.value = config.storagePath || '';

          if (config.transports) {
              confClearnet.checked = config.transports.clearnet;
              confI2P.checked = config.transports.i2p;
              confTor.checked = config.transports.tor;
          }

          if (config.storage) {
              confErasure.checked = config.storage.enableErasure;
              confDataShards.value = config.storage.defaultDataShards;
              confParityShards.value = config.storage.defaultParityShards;
          }
      } catch (e) {}
  }
  ...
  btnSaveConfig.addEventListener('click', async () => {
      const newConfig = {
          nodeName: confName.value,
          storagePath: confPath.value,
          transports: {
              clearnet: confClearnet.checked,
              i2p: confI2P.checked,
              tor: confTor.checked
          },
          storage: {
              enableErasure: confErasure.checked,
              defaultDataShards: parseInt(confDataShards.value),
              defaultParityShards: parseInt(confParityShards.value)
          }
      };
      ...
      try {
          ...
          if (res.ok) {
              configStatus.textContent = 'Saved! (Restart required for some changes)';
