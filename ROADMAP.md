...
- Added a durable JSON-backed torrent registry (`torrents.json`) so the seeding list survives node restart.
- Added a durable SQLite-backed publication registry for manifests and shards, plus a new `GET /assets` discovery API.
...
- The first health/retry/cooldown layer now exists and safe reconciliation analysis is now available; next focus is stronger peer gossip, richer backoff tuning, and heavier reconciliation execution paths.
- Extend catch-up semantics beyond ordered block replay toward richer lag diagnostics and more explicit operator workflows if multi-node deployments become heavier.
