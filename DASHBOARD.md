...
|---|---|---:|---|
| Bobcoin | `bobcoin/` | Active | Frontend + game stack remains the primary UI reference; now includes Go storage WASM workbench, publication flow, browser-side retrieval/reconstruction, signed Go-lattice manifest anchoring, a searchable trust-aware Vault archive browser, archive reuse inside Market/Gallery flows, signed publisher provenance metadata, degraded recovery diagnostics, and saved/grouped archive workflows (`v8.22.0`). |
