...
- Bobcoin frontend integration is now partially live: the React app contains a browser-side Go WASM workbench for storage preprocessing and publication, and the Go supernode can serve the required WASM runtime artifacts directly.
...
- The Bobcoin WASM workbench can now publish to the Go supernode registry, but publication is not yet anchored on the lattice and retrieval UX is still missing.
