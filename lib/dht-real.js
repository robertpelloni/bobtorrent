  ...
  async relaySignedPut (publicKey, sequence, value, signature) {
      return new Promise((resolve, reject) => {
          const opts = {
              ...
              v: value,
              sign: (buf) => {
                  // Verify that the buffer matches what was signed?
                  // bittorrent-dht creates the buffer to be signed.
                  // If we just return the signature, we assume bittorrent-dht constructed the exact same buffer.
                  // (which it should, as it's standard bencode(seq, v, salt...)).
                  return signature
              }
          }
