...
const MSG_OK = 0x08
const MSG_DHT_QUERY = 0x09
const MSG_DHT_RESPONSE = 0x0A
...
function encryptStream (socket, isServer, onMessage) {
  ...
  const handleEncryptedData = (data) => {
    ...
    while (true) {
      ...
      if (!success) {
        console.error('[SecureTransport] Decryption failed')
...
export function startSecureServer (storageDir, port = 0, onGossip = null, dht = null, samSession = null, host = '0.0.0.0') {
  const handleConnection = (socket) => {
    console.log(`[SecureServer] Incoming connection from ${socket.remoteAddress}:${socket.remotePort}`)
    const secure = encryptStream(socket, true, async (type, payload) => {
      if (type === MSG_HELLO) {
        ...
        console.log(`[SecureServer] Received REQUEST for ${blobId}`)
        const filePath = path.join(storageDir, blobId)

        if (fs.existsSync(filePath)) {
          console.log(`[SecureServer] Sending data for ${blobId}`)
          const data = fs.readFileSync(filePath)
          secure.sendMessage(MSG_DATA, data)
        } else {
          console.error(`[SecureServer] Blob not found: ${filePath}`)
      ...
      } else if (type === MSG_DHT_QUERY) {
          console.log('[SecureServer] Received DHT Query via TCP')
          // Stub: Return "Not Implemented" error in Bencode
          secure.sendMessage(MSG_DHT_RESPONSE, Buffer.from('d1:y1:e5:error2:NIs5:m16:Not Implementedee'))
      }
    })
  }
...
export function downloadSecureBlob (peer, blobId, knownSequences = {}, onGossip = null, announceAddr = null, samSession = null) {
  return new Promise((resolve, reject) => {
    ...
    console.log(`[Client] Connecting to ${host}:${port} for ${blobId}...`)
    ...
    const connect = async () => {
      ...
      try {
        ...
        console.error(`[Client] Connection failed to ${peer}: ${e.message}`)
        return reject(new Error('Connection failed: ' + e.message))
      }

      const cleanup = () => socket.destroy()
      socket.on('error', (err) => {
          console.error(`[Client] Socket error for ${peer}: ${err.message}`)
          reject(err)
      })
      socket.on('close', () => {
          console.log(`[Client] Socket closed for ${peer}`)
      ...
      const secure = encryptStream(socket, false, (type, payload) => {
        if (type === MSG_DATA) {
            console.log(`[Client] Received MSG_DATA chunk: ${payload.length} bytes`)
          chunks.push(payload)
          ...
          console.error(`[Client] Received MSG_ERROR: ${payload.toString()}`)
          cleanup()
      ...
      socket.on('close', () => {
        ...
        if (hash === blobId) {
          ...
          console.error(`[Client] Integrity check failed. Got ${hash}, expected ${blobId}`)
          reject(new Error('Integrity Check Failed'))
        }
      })

      socket.once('secureConnect', () => {
        console.log(`[Client] Secure Handshake complete with ${peer}`)
