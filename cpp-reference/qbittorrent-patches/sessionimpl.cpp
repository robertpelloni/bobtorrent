    ...
    connect(m_megaSubscription, &Megatorrent::SubscriptionManager::subscriptionUpdated, this, [this](const QByteArray &, const Megatorrent::Manifest &manifest) {
        for (const auto &file : manifest.files()) {
            for (const auto &chunk : file.chunks) {
                // Queue download: ID, Size, Key, IV, Path
                // Path: Download to Downloads/Megatorrent/<ID> for now
                QString savePath = (downloadPath() / Path(u"Megatorrent"_s) / Path(chunk.id)).toString();
                Utils::Fs::forcePath(savePath); // Ensure dir exists
                m_megaDownloader->queueBlob(chunk.id, chunk.size, chunk.key, chunk.iv, savePath);
            }
        }
    });

    connect(m_megaDownloader, &Megatorrent::BlobDownloader::peersNeeded, m_megaDHT, &Megatorrent::DHTClient::findBlobPeers);
    connect(m_megaDHT, &Megatorrent::DHTClient::peersFound, m_megaDownloader, &Megatorrent::BlobDownloader::addPeers);
...
void SessionImpl::handleAlert(lt::alert *alert)
{
    try
    {
        ...
        case lt::dht_get_peers_reply_alert::alert_type:
            if (m_megaDHT) m_megaDHT->handleDhtAlert(alert);
            break;
#ifdef QBT_USES_LIBTORRENT2
...
bool SessionImpl::addMegatorrentSubscription(const QString &publicKey, const QString &label)
{
    if (!m_megaSubscription) return false;
    // Basic hex validation
    QByteArray key = QByteArray::fromHex(publicKey.toLatin1());
    if (key.size() != 32) return false;

    invokeAsync([this, key, label] {
        m_megaSubscription->addSubscription(label, key);
        m_megaSubscription->save((specialFolderLocation(SpecialFolder::Config) / Path(u"megatorrent_subscriptions.json"_s)).toString());
    });
    return true;
}
...
bool SessionImpl::removeMegatorrentSubscription(const QString &publicKey)
{
    ...
    invokeAsync([this, key] {
        m_megaSubscription->removeSubscription(key);
        m_megaSubscription->save((specialFolderLocation(SpecialFolder::Config) / Path(u"megatorrent_subscriptions.json"_s)).toString());
    });
    return true;
}
